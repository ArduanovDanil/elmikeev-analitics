<?php

namespace App\Console\Commands\ParseApi;

use App\Jobs\StockJob;
use App\Models\Stock as StockModel;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Http;

class Stocks extends Command
{

    protected string $key = 'E6kUTYrYwZq2tN4QEtyzsbEBk3ie';
    protected string $url = 'http://109.73.206.144:6969/api/stocks';
    protected int $limit = 500;

    protected int $pageStart = 1;

    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'app:stock';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Command description';

    /**
     * Execute the console command.
     */
    public function handle()
    {

        $lastStockModel = StockModel::orderByDesc('date')->first();
        
        $dateFrom = $lastStockModel->date ?? date('Y-m-d');

        $query = [
            'dateFrom' => $dateFrom,
            'page' => $this->pageStart,
            'key' => $this->key,
            'limit' => $this->limit,
        ];

        $response = Http::acceptJson()->get($this->url, $query);
        
        $pagesCount = $response->json('meta.last_page');

        for ($page = 1; $page <= $pagesCount; $page++)
        {
            StockJob::dispatch($this->url, $query, $page)->delay(now()->addSeconds($page * 2));
        }

    }
}
